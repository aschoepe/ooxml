...
: Imports the given *part* from the file *docx* into the docx object,
  ...
  If the imported part has its own relationships part
  (*.../_rels/<part>.rels*), that relationships part is imported as well.
  Internal relationship targets reachable from the imported part are
  copied recursively into the destination package and Targets are rebased
  as needed to avoid part-name collisions.
...
: Modifies the document settings (word/settings.xml). Settings may be
  ...
  Many more settings are available — the method accepts any
  setting defined in ECMA-376 §17.15.1 CT_Settings. On/off
  settings use *onOffValue*. Refer to the property table in
  the source for the complete list.
...
: Creates a table cell within a **tablerow** script. The
  *creatingScript* creates the cell content (paragraphs, nested
  ...
  fill (same as paragraph shading).
